...
def parse(opt_path, is_train=True):
    ...
    # ----------------------------------------
    # initialize opt
    # get json config path and set training phase
    # ----------------------------------------
    ...
    # ----------------------------------------
    # set default
    # If merge_bn is not set, disable BatchNorm merging by default
    # When enabled, merges BatchNorm layers into convolution layers from the given startpoint
    # Do not use for trainign: Merging fixes normalisation parameters which stops adaptive behavior 
    # and harms training performance.
    # ----------------------------------------
    ...
    # ----------------------------------------
    # path
    # Force model to use Absolute paths
    # Create paths for storing logs, configs, train and test results
    # ----------------------------------------
    for key, path in opt['path'].items():
        if path and key in opt['path']:
            opt['path'][key] = os.path.expanduser(path) #absolute path
    ...
    # Static graphs false - can boost performance but may reduce flexibility
    # dist false - will train on a single machine or non-distributed setup.
    # ----------------------------------------
    ...
    # ----------------------------------------
    # # Default settings for perceptual loss:
    # Uses VGG feature layer 34 to extract high-level semantic features.
    # Applies L1 loss for perceptual similarity, robust to outliers.
    # Enables input normalisation to match VGG training stats.
    # Disables range normalisation to avoid unnecessary rescaling.
    # ----------------------------------------
    ...
    # G_optimizer_type: uses 'adam' for stable and adaptive learning.
    # G_optimizer_betas: standard β values for Adam (0.9, 0.999).
    # G_scheduler_restart_weights: set to 1 for equal weighting across restarts.
    # G_optimizer_wd: weight decay set to 0 (no L2 regularisation).
    # G_optimizer_reuse: False to avoid reusing previous optimiser state.
    # D_optimizer_reuse: False for discriminator optimiser if used.
    # Helps ensure reproducibility and prevents silent misconfigurations.
    # ----------------------------------------
    ...
    # True = enforce that model parameters match exactly with loaded checkpoint
    # helps catch unexpected mismatches during model loading.
    # ----------------------------------------
    ...
    # ----------------------------------------
    # Exponential Moving Average - controls smoothing of encoder weights during training
    # 0 = no smoothing 
    # higher values (close to 1) = slow, stable updates improving performance,
    # lower values = faster updates but less stable averaging.
    ...
    # net_type: 'discriminator_patchgan' (common PatchGAN architecture)
    # in_nc: input channels, typically 3 for RGB images
    # base_nc: base number of filters (64)
    # n_layers: number of convolutional layers (3)
    # norm_type: normalization type, 'spectral' stabilises GAN training
    # ----------------------------------------
    if 'netD' in opt:
...
def find_last_checkpoint(save_dir, net_type='G', pretrained_path=None):
    ...
    How it works:
        Scans save_dir for files matching '*_<net_type>.pth'
        Extracts iteration numbers from filenames using regex
        Returns the checkpoint with the highest iteration number
        If none found, returns 0 and pretrained_path as fallback.    
    """
